...
export class HLSService extends HLSController implements OnModuleInit {
    ...
    public getMasterRendition (playback: Playback, session: CachedSession) {
        return this.getFilePath(playback).fromPromise((filePath) => this.getMasterPlaylist(filePath, session.session));
    }
    ...
    public getAudioRenditionPlaylist (
        ...
        return this.getFilePath(playback).fromPromise((filePath) => this.getIndexPlaylist(
            filePath,
            session.session,
            StreamType.AUDIO,
            options.quality,
    ...
    public getVideoSegmentStream (
        ...
        return this.getFilePath(playback)
            .fromPromise((filePath) => this.getSegmentStream(
                filePath,
                session.session,
                StreamType.VIDEO,
                options.quality,
    ...
    public retrieveVTTSubtitle (playback: Playback, streamIndex: number) {
        ...
        return this.getFilePath(playback).chain((filePath) => buildSubtitle(filePath));
    }
    ...
    public generateThumbnail (video: Video, options: ScreenShotOptions) {
        return this.getFilePathFromVideo(video)
            .fromPromise((filePath) => this.generateScreenshot(
                filePath,
                options.quality,
                options.streamIndex,
                options.timestamp,
