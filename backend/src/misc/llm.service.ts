...
export class LLMService {
    ...
    static generateMediaVectorStore (
        db: PrismaService,
        openAIApiKey: string,
    ): PrismaVectorStore<MediaEmbeds, 'MediaEmbeds', any, any> {
        return PrismaVectorStore.withModel<MediaEmbeds>(db).create(
    ...
    saveMediaEmbed (media: ScanResult, mediaId: string) {
        ...
        return TaskEither.tryCatch(
            ...
            .fromPromise((docs) => this.prisma.addDocuments([docs]))
            .map(() => mediaId);
    }

    searchForMostSimilarMedia (
        query: string,
        limit: number,
        ...
        return TaskEither.tryCatch(
            ...
            .fromPromise((mediaIds) => this.db.media.findMany({
                where: {
                    AND: [
                        {
                            id: {
    ...
    generateMediaRecommendations (
        ...
        return this.getEmbeddings(mediaId).chain(({ vector }) => TaskEither.of(vector)
            .matchTask<Media[]>([
                {
    ...
    openAIKeyUpdated (event: OpenAIKeyEvent) {
        try {
            this.prisma = LLMService.generateMediaVectorStore(
                this.db,
                event.openAIKey,
            );
        } catch (error) {
