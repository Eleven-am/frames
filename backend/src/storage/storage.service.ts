import { TaskEither, createInternalError, Either } from '@eleven-am/fp';
...
import { CloudDrive, CloudStorage, Folder, Media, MediaType, User, Video } from '@prisma/client';

import { CreateStorageArgs, storageCredentialsSchema, UpdateStorageArgs } from './storage.args';
...
export class StorageService {
    ...
    getObjectFromMedia (media: Media) {
        return TaskEither
            ...
            .filter(
                (media) => (media.type === MediaType.SHOW && media.folder !== null) || (media.type === MediaType.MOVIE && media.videos.length === 1),
                () => createInternalError('Media structure is invalid'),
            )
            .map((media) => ({
                storageId: media.folder?.cloudStorageId || media.videos[0].cloudStorageId,
    ...
    getStreamDataFromVideo (video: Video, range: string) {
        ...
        return this.performActionById(video.cloudStorageId, (provider) => provider.streamFile(video.location, newRange));
    }

    getContents (cloudStorage: CloudStorage, folder?: string) {
        return this.performAction(cloudStorage, (provider) => provider.readFolder(folder))
            .map((items) => items.map((item): FramesFile => ({
                ...item,
    ...
    readFolder (cloudStorageId: string, path?: string) {
        return this.performActionById(cloudStorageId, (provider) => provider.readFolder(path))
            .map((items) => items.map((item): FramesFile => ({
                ...item,
                cloudStorageId,
            })));
    ...
    getRecursiveContents (cloudStorage: CloudStorage, folder: string): TaskEither<RecursiveFramesFile[]> {
        return this.getStorage(cloudStorage)
            .chain(({ provider: storageService }) => TaskEither
                .tryCatch(
                    () => storageService.getFileOrFolder(folder),
                    'Failed to read folder',
                )
                .chain((folder) => TaskEither
                    .tryCatch(
                        () => storageService.readFolder(folder.path),
                        'Failed to read folder',
                    )
                    .map((items) => {
                        const recursiveItems = items.map((item): RecursiveFramesFile => ({
                            ...item,
                            cloudStorageId: cloudStorage.id,
                            parentName: folder.name,
                        }));

                        const files = recursiveItems.filter((item) => !item.isFolder);
                        const folders = recursiveItems.filter((item) => item.isFolder);
            ...
            .chain(({ files, folders }) => TaskEither
                .of(folders)
                .chainItems((folder) => this.getRecursiveContents(cloudStorage, folder.path))
                .map((items) => items.flat())
                .map((items) => [...files, ...items]));
    }

    create (user: User, createStorageDto: CreateStorageArgs) {
        return Either
            .tryCatch(() => JSON.parse(createStorageDto.credentials), 'Invalid credentials')
            .parseSchema(storageCredentialsSchema)
            .toTaskEither()
            .map((credentials): Omit<CloudStorage, 'id' | 'updated' | 'modify' | 'created' | 'userId'> => ({
                ...createStorageDto,
                credentials,
            }))
    ...
    getFile (cloudStorageId: string, location: string) {
        return this.performActionById(cloudStorageId, (provider) => provider.getFileOrFolder(location))
    ...
    deleteFileOrFolder (cloudStorageId: string, location: string) {
        return this.performActionById(cloudStorageId, (provider) => provider.deleteFileOrFolder(location));
    }

    private getStorageFromId (cloudStorageId: string) {
        return TaskEither
            .tryCatch(
                () => this.prismaService.cloudStorage.findUnique({ where: { id: cloudStorageId } }),
    ...
    private getStorage (cloudStorage: CloudStorage) {
        const getLocal = (cloudStorage: CloudStorage) => this.baseStorageService
            .createProvider({
                provider: Provider.LOCAL,
                options: cloudStorage.credentials as LocalStorageOption['options'],
            });

        const getS3 = (cloudStorage: CloudStorage) => this.baseStorageService
            .createProvider({
                provider: Provider.S3,
                options: cloudStorage.credentials as S3StorageOption['options'],
            });

        const dropbox = (cloudStorage: CloudStorage) => this.baseStorageService
            .createProvider({
                provider: Provider.DROPBOX,
                options: cloudStorage.credentials as DropboxStorageOption['options'],
            });

        const googleDrive = (cloudStorage: CloudStorage) => this.baseStorageService
            .createProvider({
        ...
        return Either
            ...
            .match([
                {
                    predicate: (cloudStorage) => typeof cloudStorage.credentials === 'string',
                    run: () => {
                        cloudStorage.credentials = JSON.parse(cloudStorage.credentials as string);

                        return cloudStorage;
                    },
                },
                {
                    predicate: (cloudStorage) => typeof cloudStorage.credentials === 'object',
            ...
            .match([
                ...
                {
                    predicate: (cloudStorage) => cloudStorage.drive === CloudDrive.DROPBOX,
                    run: dropbox,
                },
                {
    ...
    private testConnection (cloudStorage: CloudStorage) {
        return this.getStorage(cloudStorage)
            .chain(({ provider }) => TaskEither.tryCatch(() => provider
                .getFileOrFolder(cloudStorage.movieLocations[0]), 'Failed to test connection'))
            .map(() => 'Connection successful');
    }

    private performActionById <ReturnType> (cloudStorageId: string, action: (storageService: BaseStorageService) => Promise<ReturnType>) {
        return this.getStorageFromId(cloudStorageId)
            .chain(({ provider }) => TaskEither.tryCatch(() => action(provider), 'Failed to perform action'));
    }

    private performAction <ReturnType> (cloudStorage: CloudStorage, action: (storageService: BaseStorageService) => Promise<ReturnType>) {
        return this.getStorage(cloudStorage)
            .chain(({ provider }) => TaskEither.tryCatch(() => action(provider), 'Failed to perform action'));
    }
