...
const PlayButton = ({id, name, url}: ButtonInterfaces) => {
    ...
    return (
        <Link href={url || '/watch?mediaId=' + id}>
...
const PLayList = ({id}: ButtonInterfaces) => {
    const {openPlaylist} = usePlaylist();

    return (
        <button className={`${styles.roundGuys} ${styles.noFill}`} title={'add to playlist'} onClick={() => openPlaylist(id)}>
...
const GroupWatch = ({id}: ButtonInterfaces) => {
    ...
    return (
        <button title={'GroupWatch'} className={styles.roundGuys} onClick={() => openSession({id})}>
...
const Rating = ({id, review, myRating}: ButtonInterfaces) => {
    ...
    return (
        ...
        >
            ...
            <div className={styles.review} onClick={getRating}>
...
const InfoButton = ({id, name, type}: ButtonInterfaces) => {
    ...
    return (
        <Link href={'/info?mediaId=' + id} as={'/' + type + '=' + name?.replace(/\s/g, '+')}>
...
export const HoverContainer = ({children, hovering}: { children: React.ReactNode, hovering: (b: boolean) => void }) => {
    const hoverHandler = useRef<(b: boolean) => void>(hovering);
    const [hover, setHover] = useState(false);

    useEffect(() => {
        hoverHandler.current = hovering;
    }, [hovering]);

    useEffect(() => {
        hoverHandler.current(hover);
    }, [hover]);
