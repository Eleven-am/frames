...
export default function LeftControls() {
    ...
    const setShareAndDownload = useSetRecoilState(shareAndDownloadAtom);
    ...
    return (
        <div style={isTabOrMobile() || left ? {opacity: 1} : {}} className={`${styles.a} ${styles.left}`}
             onClick={e => e.stopPropagation()}>

            {airplay.available ?
                <button className={airplay.casting ? `${styles.nf} ${styles.con}` : styles.nf}
                        onClick={playback.castToDevice}>
            ...
            <button onClick={playback.muteUnmute} className={mute ? `${styles.f} ${styles.muted}` : styles.f}>
                <svg viewBox="0 0 24 24">
            ...
            {user && user.role !== Role.GUEST ?
                <button className={`${styles.f} ${styles.download}`}
                        onClick={() => setShareAndDownload(down => ({...down, download: true}))}>
