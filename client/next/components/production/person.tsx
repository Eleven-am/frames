...
function MinHolder({data, person}: { data: number, person: PersonInterface }) {
    ...
    return (
        <div className={styles.bar}
             style={{background: `rgba(1, 16, 28, ${values.lowOpacity > 0.7 ? 0.7 : values.lowOpacity})`}}
             onScroll={onScroll}>
            ...
            <div className={styles.holder}>
                ...
                {person.castMedia.length ? <SectionBone data={person.castMedia} type={'BASIC'}
                                                        display={'media ' + person.name + ' starred in'}/> : null}
                {person.writtenMedia.length ? <SectionBone data={person.writtenMedia} type={'BASIC'}
                                                           display={'media written by ' + person.name}/> : null}
                {person.directedMedia.length ? <SectionBone data={person.directedMedia} type={'BASIC'}
                                                            display={'media ' + person.name + ' directed'}/> : null}
                {person.producedMedia.length ? <SectionBone data={person.producedMedia} type={'BASIC'}
                                                            display={'media ' + person.name + ' produced'}/> : null}
            </div>
...
export const ErrorPage = ({error, offline}: { error: ErrorProps, offline?: boolean }) => {
    ...
    return (
        <div className={styles.bar2}>
            <HoverContainer onHover={hovering}>
                {offline ?
...
export default function Holder({person}: { person: PersonInterface }) {
    const {
        ...
        loading,
        error
    } = useFetcher<PlayListResponse | null>('/api/media/personPlaylist?mediaId=' + person.id);
