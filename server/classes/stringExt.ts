...
declare global {
    ...
    interface String {
         ...
         * @param string - the string to compare to
         * @param whole - if true, returns the amount of words the original shares with the new string
         */
...
String.prototype.Levenshtein = function (string: string, val?: number): any {
    ...
    for (i = 1; i <= b.length; i++) {
        for (j = 1; j <= a.length; j++) {
            m[i][j] = b.charAt(i - 1).toLowerCase() === a.charAt(j - 1).toLowerCase() ? m[i - 1][j - 1] : m[i][j] = min(m[i - 1][j - 1] + 1, min(m[i][j - 1] + 1, m[i - 1][j] + 1))
        }
    }
...
String.prototype.strip = function (string: string, whole?: boolean): boolean {
    ...
    for (let i = 0; i < first.length; i++) for (let j = index + 1; j < second.length; j++) {
...
export class RestAPI {
    ...
    public decrypt<S>(salt: string, encoded: string) {
        ...
        try {
            const response = JSON.parse((Buffer.from(encoded, "base64").toString("utf-8").match(/.{1,2}/g) || [])
